...
To use the `Sandwich` package, first install it from source code (only do this once).
```{r}
...
Then load the installed package into your environment using `library()`. Make sure to capitalize the first letter of **S**andwich. Remember to load the package every time you use it.
```{r}
